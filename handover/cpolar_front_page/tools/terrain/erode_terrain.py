...
OUT = os.path.join(HERE, "eroded_terrain.npy")
...
# ------------------------------------------------------------------ sharpen the ridges
# Blown up 4x beside the reference, the difference was not texture: the reference has
# ARETES - sharp ridge lines running down from the summit with distinct faces between
# them - and this had one smooth cone with streaks down it, like a pile of sand. Every
# stage before this point smooths: the resample, the anti-alias blur, the spike blur.
# Ridges are the first thing a blur takes and the last thing anyone notices is missing.
#
# An unsharp mask puts them back. Subtracting a blurred copy leaves exactly the crests
# and the hollows, and adding that back raises every ridge and deepens every valley
# without inventing a feature that was not already in the survey data.
for radius, amount in ((6.0, 0.55), (2.4, 0.40), (1.0, 0.22)):
    s = s + amount * (s - ndimage.gaussian_filter(s, radius))
s = np.clip(s, -25.0, 130.0)
